window.onload = function() {
	...
	const roundForFreeDebate = 5, firstSpeak = 1;	//自由辩环节； 自由辩时先发言方
	var round = 1,		//第几环节(从1开始) and many other more function
		...
		freeDebateBtn = document.getElementById('freeDebate');	//自由辩按钮
			...
			if (this.textContent === 'Start') {
				...
				flag[num] = setInterval(function() {		//该函数用于 输出计时情况
					...
					if(count[num] > 0) {
						...
						if (round === roundForFreeDebate) {
							freeDebate();		//该自由辩啦！掌声起！
							backToNormalDebate();
						}
					}
				}, 1000 / beishu);
	...
	function freeDebate() {
		...
		freeDebateBtn.style.visibility = 'visible';
		freeDebateBtn.textContent = 'Start';
	}
	
	function backToNormalDebate() {
		button[0].style.visibility = 'visible';
		button[1].style.visibility = 'visible';
		freeDebateBtn.style.visibility = 'hidden';
